...
file_path_1 = './conf_A_2D_result.txt'
file_path_2 = '../run_conf_B_dual_QWEN_test/conf_B_2D_result.txt'
...
# 高斯平滑
sigma = 1.0  # 设置高斯平滑的标准差
hist_2 = gaussian_filter(hist_2, sigma=sigma)
...
# 使用plot_surface绘制平滑曲面图
surf_2 = ax_3.plot_surface(x_grid_2, y_grid_2, hist_2.T, cmap='coolwarm', rstride=1, cstride=1, antialiased=True, alpha=0.8, vmin=0, vmax=1.0)
...
ax_3.set_xlabel('\nConf_B binding affinity  (kcal/mol)')
ax_3.set_ylabel('\nConf_A binding affinity  (kcal/mol)')
ax_3.set_zlabel('\nProbability density')

# 在3D图形底面投影二维轮廓
# contour = ax.contour(x_grid, y_grid, hist.T, zdir='z', offset=-600, cmap='coolwarm', linewidths=1)
# 在3D图形底面投影二维轮廓并填充轮廓
contour_2 = ax_3.contourf(x_grid_2, y_grid_2, hist_2.T, zdir='z', offset=-0.70, cmap='coolwarm', levels=20, alpha=0.7, vmin=0, vmax=1.0)
...
ax_3.set_xlim(-10, -5)
ax_3.set_ylim(-10, -5)
ax_3.set_zlim(-0.7, 1.0)
ax_3.view_init(elev=12, azim=235)
