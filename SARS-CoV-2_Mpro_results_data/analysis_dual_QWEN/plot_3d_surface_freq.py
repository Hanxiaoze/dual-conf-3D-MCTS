...
# # 网格离散化
bins = 50
# hist, x_edges, y_edges = np.histogram2d(x, y, bins=bins, range=[x_range, y_range])
...
# surf = ax.plot_surface(x_grid, y_grid, hist.T, cmap='coolwarm', rstride=1, cstride=1, antialiased=True, alpha=0.8)



# # 添加标题和标签
# ax.set_title('3D distribution of the traning molecules', fontsize=12)
...
# # 在3D图形底面投影二维轮廓并填充轮廓
# contour = ax.contourf(x_grid, y_grid, hist.T, zdir='z', offset=-700, cmap='coolwarm', levels=20, alpha=0.8)
...
# ax2.hist2d(x, y, bins=bins, cmap='coolwarm', range=[x_range, y_range])
# ax2.set_title('2D projection heatmap of the traning molecules', fontsize=12)
...
# 网格离散化
hist_2, x_edges_2, y_edges_2 = np.histogram2d(x_2, y_2, bins=bins, range=[x_range, y_range])
...
# 使用plot_surface绘制平滑曲面图
surf_2 = ax_3.plot_surface(x_grid_2, y_grid_2, hist_2.T, cmap='coolwarm', rstride=1, cstride=1, antialiased=True, alpha=0.8)



# 添加标题和标签
ax_3.set_title('Generated molecules with affinity conditions Epro= -{}.0 / Mpro= -{}.0 kcal/mol'.format(arg1, arg2))
ax_3.set_xlabel('Epro binding affinity  (kcal/mol)', fontsize=10)
ax_3.set_ylabel('Mpro binding affinity  (kcal/mol)', fontsize=10)
ax_3.set_zlabel('Frequency', fontsize=10)

# 在3D图形底面投影二维轮廓
# contour = ax.contour(x_grid, y_grid, hist.T, zdir='z', offset=-600, cmap='coolwarm', linewidths=1)
# 在3D图形底面投影二维轮廓并填充轮廓
contour_2 = ax_3.contourf(x_grid_2, y_grid_2, hist_2.T, zdir='z', offset=-7, cmap='coolwarm', levels=20, alpha=0.8)
...
ax_3.set_ylim(-14, -2)
ax_3.set_zlim(-7, 10)
ax_3.view_init(elev=12, azim=255)

# # 在3D图形右方绘制二维投影热图
# ax_4 = fig.add_subplot(224)
# # im = ax_4.imshow(hist_2.T, cmap='coolwarm', extent=[x_centers_2.min(), x_centers_2.max(), y_centers_2.min(), y_centers_2.max()])
# ax_4.hist2d(x_2, y_2, bins=bins, cmap='coolwarm', range=[x_range, y_range])
# ax_4.set_title('2D projection heatmap of the generated molecules', fontsize=12)
